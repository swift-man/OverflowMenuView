...
Use ``OverflowMenuView`` when you want a menu that can be controlled either by user gestures or by a parent `Binding<Bool>`. Each content closure receives an ``OverflowMenuContext`` so custom title, drawer, and main content can open, close, or react to the menu's progress. If a screen should keep the same top bar chrome but never expose the drawer, set `isMenuInteractionEnabled` to `false`.
